...
function CustomTooltip({ active, payload }: { active?: boolean; payload?: Array<{ name: string; value: number; payload: ChartDataItem }> }) {
  ...
  return (
    <div className="rounded-lg border border-[#2D3E57] bg-[#1E293B] px-3 py-2 shadow-xl text-xs">
      <p className="font-medium text-[#E2E8F0] mb-1">{item.name}</p>
      <p className="font-mono text-[#22C55E]">{formatCurrency(item.value)}</p>
    </div>
  )
}
...
export function PieChartBase({ data, height = 260, showLegend = true, innerRadius = 55 }: PieChartBaseProps) {
  if (!data.length) {
    return (
      <div className="flex items-center justify-center text-[#475569] text-sm" style={{ height }}>
